...
class RestApi
  ...
  def request_cms_api(method, path, payload)
    ...
    if response.is_a?(Hash) && response.keys == ["task"] && response["task"].is_a?(Hash)
      ...
      if task_data["status"] == "success"
        ...
        raise ScrivitoError.new("412 #{task_data["code"]} #{task_data["message"]}")
      end
    else
      response
    end
  ...
  def response_for_request_cms_api(method, path, payload)
    ...
    if response.code.start_with?("2")
      ...
      raise ScrivitoError.new("#{response.code} #{response.body}")
    end
  end
  ...
  def internal_retry(request_proc, timeout, backoff_wait_time)
    ...
    if response.code == "429"
      if Time.now < timeout
        ...
        raise ScrivitoError.new("429 rate limit exceeded")
      end
    else
      response
    end
